...
router.post("/query", (req, res) => {
  ...
  MongoClient.connect(url, { useUnifiedTopology: true }, async (err, client) => {
    if (!err) {
      ...
      collection_topics.find({ $text: { $search: req.body.query } }).toArray(function(err, todos) {
        if (!err) {
            // send output back
            console.log(todos);
        //   if (todos[0] == undefined) return res.send([undefined,"No Data Found"]);
          res.send(todos);
        }
      });;
    }
...
//           name:
//             req.body.new_topic[0].toUpperCase() + req.body.new_topic.slice(1),
//         });
